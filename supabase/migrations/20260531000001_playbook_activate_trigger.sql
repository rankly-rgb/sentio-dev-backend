...
BEGIN
  ...
  IF NEW.status = 'active' AND (OLD.status IS DISTINCT FROM 'active') THEN
    ...
    -- Signature exacte : net.http_post(url, body jsonb, params jsonb, headers jsonb, timeout_milliseconds)
    -- Paramètres positionnels — les paramètres nommés (:=) ne sont pas supportés sur ce projet.
