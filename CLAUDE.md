...
**Churn Risk** (`calcChurnRiskV2` / `buildChurnSignals`) — additif, **découplé du Health Score** (plus de `100 - health`). 8 signaux déterministes (jamais de probabilité/confidence) : `invoice_overdue_15d`(35, CRITIQUE), `payment_delinquent`(35, CRITIQUE — `accounts.is_delinquent`, mutuellement exclusif avec `invoice_overdue_15d` : même fait observé à deux précisions différentes, voir décision ci-dessous), `mrr_contraction_20pct_3mo`(30, CRITIQUE), `payment_failures_90d`(25, MAJEUR), `monthly_young_account`(20, MAJEUR), `annual_renewal_soon_with_contraction`(20, MAJEUR), `plan_downgrade_6mo`(10, MINEUR), `invoice_overdue_under_15d`(10, MINEUR). Un signal dont la donnée est absente est *skippé* (`value: null`), jamais compté comme non-déclenché — `payment_delinquent` fait exception : `is_delinquent` est un booléen `NOT NULL`, toujours évaluable, c'est tout son intérêt (couvre la fenêtre où les signaux factures ne peuvent pas encore répondre). Band `low`/`watch`/`high` (seuils 25/50).
...
3. `impayes` — factures en retard **ou** `is_delinquent` (élargi 2026-08-06, décision 2 : un compte délinquent EST un compte en impayé, que la preuve vienne d'une facture ou du statut de souscription Stripe — la distinction est un détail d'implémentation, pas quelque chose que l'utilisateur doit voir dans sa segmentation. `impayes` reste prioritaire sur `en_danger_critique`/`a_risque_leger` dans la chaîne : un compte délinquent qui y aurait atterri bascule désormais en `impayes`, délibérément — « impayé » dit quoi faire, « à risque » non)
...
### Décision actée — délinquence, un état actionnable, pas seulement un signal de risque (2026-08-06)

Audit délinquence 2026-08-06 : 164 comptes `is_delinquent = true` sur le portefeuille, tous en bande `low`, zéro en `watch`/`high`, tuile Overview « Accounts at risk 0 ». Le champ D-NEXT était correctement écrit depuis le 2026-08-04 mais jamais lu par le scoring — voir Incident #6, `IMPLEMENTATION_LOG.md`. Objectif métier : un compte dont le prélèvement a échoué doit être visible et actionnable le jour même.

**Décision 1 — cumul avec les signaux facture** : `is_delinquent` devient le signal `payment_delinquent` (35pts, CRITIQUE), en **exclusion mutuelle** avec `invoice_overdue_15d` (même fait — un paiement en échec — observé à deux niveaux de précision ; une fois l'invoice confirmée, le proxy s'efface au lieu de s'additionner). `payment_failures_90d` reste pleinement indépendant (axe distinct : compte d'échecs sur 90j, pas un statut instantané). Plafond du cluster « détresse de paiement » ramené de 95 à 60pts.

**Décision 2 — segment `impayes`** : voir Segmentation V3 ci-dessus, `impayes` élargi à `hasOverdueInvoices || isDelinquent`.

**Décision 3 — surfaces d'action, jamais via un seuil numérique** : un compte uniquement délinquent (35pts) ne franchit jamais un seuil conçu pour autre chose (`churn_risk_score > 70`, `>= 70` dans `computePriority`). `is_delinquent` est câblé en **condition OR directe** sur chaque surface qui parle de « comptes à risque » : tuile Overview `accounts_at_risk`/`mrr_at_risk_cents` (`dashboard-api`), `get_portfolio_snapshot.at_risk_count` (RPC, seul consommateur `get-today-status`), `accounts_with_priority.priority_label` (nouvelle branche `is_delinquent → 'critical'`), `computePriority` de `get-today-actions` (`is_delinquent → P0`). Piège identifié avant implémentation : la majorité des comptes délinquents ont `mrr_status='unavailable'` (`mrr_cents=0` par exclusion de devise minoritaire) — `mrr_at_risk_cents` ne somme que le sous-ensemble chiffrable, `accounts_at_risk_unpriced` expose le nombre exclu plutôt que de laisser un total silencieux passer pour « rien à risque en argent ».

`AccountData` (`_shared/playbook-engine.ts`) et le `SELECT` de `playbook-scheduler` gagnent `is_delinquent` — sans ça, un playbook de dunning n'a aucun champ à cibler.

**Hors périmètre, explicitement** : `delinquent_since` (pas d'horodatage aujourd'hui — un impayé du jour et un impayé de 40 jours scorent identiquement, issue dédiée à ouvrir) ; couverture de sync des factures (issue #34, déjà ouverte, distincte) ; `p0_insights_count`/`generate-insights` (un impayé devrait à terme générer un insight dédié — issue à ouvrir) ; `onboarding-first-win`/`onboarding-status` (`health_score < 40`, axe différent, non touché).

### Décision actée — eligibility_criteria vide/absent ne matche plus rien (C2.5, 2026-08-02)

Un playbook sans `eligibility_criteria` (ou avec `conditions: []`) ne matche plus aucun compte par défaut — avant cette décision, `evaluateConditions` traitait un groupe de conditions vide/absent comme "matche tout", et `playbook-scheduler` n'avait alors plus aucun garde-fou pour un playbook sans `segment_id` non plus : exécution silencieuse sur la totalité du portefeuille de l'org. Un ciblage volontaire "tous les comptes" doit désormais passer par `match_all: true` explicite dans le JSONB. Les playbooks existants ont été rétro-compatibilisés par migration (`20260802000003`, voir `docs/CHANGELOG_STABILITY.md`) — aucun ne s'est arrêté de s'exécuter suite à ce changement. La sélection manuelle explicite (`account_ids` dans `playbook-execute`) reste volontairement non filtrée par eligibility_criteria — une sélection humaine explicite exprime déjà l'intention.
