...
## Infrastructure de fin de chantier — pipeline, drift-check, Sentry (2026-08-15 → 2026-08-17)

Trois pièces d'un même chantier de fermeture : un déploiement production fiable et sérialisé (Pièce 1), la détection de dérive entre le repo et le projet Supabase réel (Pièce 2, cf. Lot 8 et ses suites déjà loguées ci-dessous), et une observabilité qui remonte les erreurs et les silences plutôt que de les laisser dormir dans des logs qu'il faut savoir aller lire (Pièce 3). Cette entrée couvre les trois, en particulier le diagnostic le plus long du lot : un design de check-in Sentry construit sur une hypothèse jamais vérifiée.

### Pièce 1 — pipeline de déploiement (#79)

Le guide de chantier prescrivait un nouveau workflow `link → db push → functions deploy → smoke test` sur push vers `main`. Vérification avant écriture : `supabase-deploy.yml` fait déjà exactement ça (13 étapes, smoke test `a13` en dernier, plus les garde-fous `a10`/`a11` et les assertions data-truth des Lots 1-6 déjà documentées plus bas dans ce fichier) — écrire un second workflow aurait fait courir deux `db push` concurrents sur la même base à chaque merge, et lequel des deux `functions deploy` arrive en second aurait décidé de ce qui reste live. Remplacer l'existant par la version plus courte du guide aurait, à l'inverse, perdu toutes les assertions déjà en place. Ni l'un ni l'autre n'est une amélioration : le seul écart réel était l'absence de `concurrency`/`cancel-in-progress` sur ce workflow (aucun workflow du repo n'en avait), donc deux merges rapprochés s'exécutaient réellement en parallèle. Corrigé — `cancel-in-progress: false` délibérément : une migration à moitié appliquée est pire qu'un run en attente.

**Non vérifié** : ce fichier n'étant pas dans le filtre `paths` du workflow, le merger ne déclenche pas de run — un `workflow_dispatch` reste à faire pour valider la version modifiée (le pipeline avait produit un run complet vert 15 étapes plus tôt le même jour, avant l'ajout de ce bloc).

### Pièce 2 — drift-check (#74-#78)

Détaillée dans les sections « Lot 8 » et suivantes ci-dessous (2026-08-15) : nouveau workflow `drift-check.yml`, rendu possible seulement après avoir réparé un historique de migrations jusque-là non rejouable (`cron.alter_job` sur des `jobid` en dur, `GRANT` du Lot 1 portant sur des fonctions pas encore déclarées à ce stade de l'historique). Le job « Fonctions déployées vs repo » est vert. Le job « Schéma base vs migrations » reste **rouge en connaissance de cause** (#78) — documenté explicitement plutôt que laissé rouge sans explication, pour éviter le mode de défaillance qu'un workflow rouge en permanence produit (on arrête de le regarder). Cause : bruit de privilèges par défaut posés par la plateforme Supabase (non filtré) + deux policies RLS non versionnées, toutes deux capturées au parking lot.

### Pièce 3 — Sentry (#80-#85)

**Frontend et Edge Functions (#80)** : `_shared/sentry.ts::withSentry(name, handler)` enrôle les 67 Edge Functions (import + wrap, 2 lignes chacune, transformation par parsing TypeScript réel — 16 fichiers ont du code après `Deno.serve`, un remplacement textuel de « la dernière ligne » aurait été faux sur ceux-là). Sans `SENTRY_DSN`, `withSentry` retourne le handler inchangé — comportement identique à avant l'enrôlement pour toute fonction non configurée. `defaultIntegrations: false` (évite le mélange de contexte entre requêtes qui partagent une isolate) ; `Sentry.flush(2000)` avant la réponse (une isolate peut geler dès la réponse renvoyée, perdant un événement encore en file). `sentry-selftest` (nouvelle fonction, lève systématiquement) fournit la seule preuve acceptable — un événement réellement reçu dans Sentry — sans casser un endpoint métier réel. Preuve produite : capture confirmée côté Sentry pour `sentry-selftest`, screenshot fourni par Naima.

**Cron Monitors — le diagnostic (#81-#85)**. `startCronCheckin` (`_shared/sentry-cron.ts`) pointe `nightly-sync`/`nightly-scoring` auprès de Sentry en fin d'exécution, sur l'URL dérivée du `SENTRY_DSN` déjà configuré. Trois incidents successifs, chacun avec une cause distincte :

1. **500 sur `sync-stripe` (#83)** — `startCronCheckin` était appelé sans jamais être importé dans ce fichier (l'import avait été ajouté à `calculate-scores` mais oublié ici). `ReferenceError`, chaque appel de la fonction retournait 500, entre 18:38 et ~20:20 UTC le 2026-08-16. Diagnostiqué en quelques minutes grâce au `console.error` ajouté par #82 dans `withSentry` — sans lui, `sentry-selftest` aurait été la seule fonction à laisser une trace lisible, et ce 500-ci serait resté un mystère de logs Supabase bruts. Root cause confirmée par la stack exacte (`sync-stripe/index.ts:832:7`), fix d'une ligne.

2. **Check-in jamais refermé, "Timed Out" sur des runs pourtant réussis (#84, #85)** — le design initial (#81) reposait sur un protocole ouverture/fermeture : POST au démarrage (`in_progress`), PUT à la fin (`ok`/`error`), réutilisant l'`id` que la réponse du POST était supposée renvoyer. **Cette hypothèse n'avait jamais été vérifiée**, et #81 la qualifiait même explicitement de « piège principal de cette pièce » — en pariant sur le mauvais piège. Vérifié en direct le 2026-08-17 (instrumentation ajoutée par #84, log `1226c5a4`/`nightly-sync`, run de 18 secondes parfaitement réussi mais marqué `Timed Out`) : **l'API de check-in Sentry répond `202` avec un corps vide, elle ne rend aucun `id`.** Un pointage d'ouverture crée donc un check-in qu'aucun pointage de fermeture ne peut jamais refermer par id — il expire au timeout du monitor, produisant une fausse alerte à chaque exécution pourtant réussie.

   **Correction de conception, pas un contournement (#85)** : protocole réduit à un seul pointage, en fin de run (`{"status": "ok"|"error"}`), plus aucun `in_progress`, plus aucune dépendance au corps de la réponse — la classe de bug disparaît au lieu d'être rapiécée. Coût assumé : Sentry ne voit plus un run « en cours », donc plus de détection de dépassement de durée (`max_runtime`). Préservé — la raison d'être de la pièce : un run planifié qui n'arrive pas à l'heure reste détecté puisque c'est l'**absence** de pointage qui alerte ; un run qui meurt en route n'envoie rien et devient un « missed », ce qui décrit mieux la réalité qu'un check-in resté ouvert indéfiniment.

**Preuve dans les deux sens, observée en direct le 2026-08-17** :
- Run réussi → check-in `Okay`, colonne STARTED "Not Sent" (comportement attendu du protocole à un seul pointage), issue Sentry précédente (`#65444749`) auto-résolue, bandeau monitor "production ✓".
- Run qui meurt en cours de route (`calculate-scores`, cf. entrée séparée ci-dessous sur le déluge de rate limits) → aucun check-in envoyé, absence détectée par le monitor comme un run manqué — exactement le comportement voulu, découvert d'ailleurs *par* ce premier pointage réel (le déluge de rate limits nocturne était invisible avant que quelque chose n'attende activement un signal de fin de run).

**Tests** : `supabase/tests/sentry-cron.test.ts` réécrit pour le protocole à un seul pointage (16 tests) — dont le plus important : un `202` au corps vide ne produit **aucun** `console.warn` (c'est la réponse normale de Sentry ; la traiter comme une anomalie serait le prochain faux signal). Couvre aussi : rien n'est émis avant `finish()`, statut `error` transmis tel quel, pointage rejeté tracé (status + corps), fetch qui rejette, fetch qui lève de façon synchrone, `Deno.env.get` qui lève, double `finish()` idempotent (une seule requête HTTP au total).

**Décision confirmée par Naima (2026-08-17)** — `self-monitor` : gardé tel quel, fichier inchangé. Ses 3 blocs d'alerte (croissance DLQ, scoring périmé, échecs de sync) sont désormais redondants avec les Cron Monitors et mieux couverts par eux ; ses 3 tâches de ménage (auto-fail `data_syncs`/`playbook_executions` bloquées, purge DLQ > 30j) n'ont pas d'équivalent Sentry et restent nécessaires — parkées pour réécriture future (`PARKING_LOT.md`). Issue #38 (à l'origine du chantier cron jobs du 2026-08-10, cf. entrée plus bas) fermée sur ce motif.

**Non vérifié dans ce chantier** : aucun test end-to-end n'a exercé un run `calculate-scores` réussi de bout en bout après #85 (le run observé pendant la fenêtre de preuve est mort en route, cf. ci-dessus) — la preuve « run réussi → Okay » repose sur `nightly-sync` uniquement. `calculate-scores` meurt en pratique toutes les nuits depuis au moins le 2026-08-03 (déluge de rate limits sur les dispatches fire-and-forget, `outbound-webhook-dispatch`/`playbook-executor`) — révélé par ce chantier, volontairement non ouvert ici (`PARKING_LOT.md`, 2026-08-16).
...
**Source de `delinquent_since` — DÉCISION AUTONOME (écart au plan initial)** : la priorité à 3 niveaux prévue (`invoice.status_transitions` → `invoice.due_date` → `subscriptions.current_period_start`) n'est réalisable qu'au 3e niveau. `accounts.is_delinquent` est produit par `aggregateAccountMrr` (`_shared/mrr-engine.ts`), une fonction **pure sans accès aux invoices** ; et `sync-stripe` ne les a de toute façon pas encore fetchées à ce stade du pipeline (`syncCustomers` → `syncSubscriptions` → `syncInvoices`). Seul le 3e niveau (`current_period_start`, déjà en mémoire dans `accountSubMeta` sous `contractStart`) est architecturalement disponible sans restructuration disproportionnée de ce chantier. Simplification assumée, cohérente avec S1 : jamais de date fabriquée — `null` si aucune subscription délinquente n'a de `contractStart` connu.

**Changements :**
- Migration `20260813000006_delinquent_since.sql` : `accounts.delinquent_since` (DATE, nullable, défaut `NULL` — jamais `now()`), index partiel `WHERE delinquent_since IS NOT NULL`. CHECK constraints `accounts_churn_risk_band_check`/`score_history_churn_risk_band_check` élargies (`'low'/'watch'/'high'/'churned'` → `+'critical'`).
- `_shared/mrr-engine.ts` : `computeDelinquentSinceCandidate(subs)` — plus ancien `contractStart` parmi les subscriptions actuellement `past_due`/`unpaid`. `resolveDelinquentSince(isDelinquentNow, previousDelinquentSince, candidateSinceIso)` — résolution "sticky" : ne redescend jamais à `null` tant que le compte reste délinquent, ne rajeunit jamais une date déjà connue, comble le trou si une date apparaît sur un run ultérieur. Vit hors d'`aggregateAccountMrr` (fonction pure sans DB) — c'est l'appelant qui connaît la valeur déjà persistée.
- `sync-stripe/index.ts` : pre-fetch accounts étendu à `is_delinquent, delinquent_since` (map `prevDelinquentSinceByAccount`, même pattern que `prevMrrByAccount`) ; candidat + résolution sticky calculés par compte, `delinquent_since` ajouté à la ligne d'upsert.
- `stripe-webhook/index.ts` : même câblage, mais DÉCISION AUTONOME distincte — `subscriptions` ne persiste pas `current_period_start` par ligne (seulement `accounts.contract_start_date`, écrasé par la subscription "primaire"), et un event webhook ne porte que LA subscription qui l'a déclenché, jamais la liste complète du compte. Candidat limité à cette subscription, uniquement si elle est elle-même délinquente ; sinon `null` (jamais une date fabriquée). `resolveDelinquentSince` (sticky) compense : une date déjà captée par un run `sync-stripe` (vision complète) est préservée telle quelle par ce chemin partiel.
- `_shared/scoring.ts` : `applyDelinquencyBandFloor(rawBand, isDelinquent, delinquentDurationDays)` — grille `NULL`/0-14j → floor `watch`, 15-44j → floor `high`, ≥45j → floor `critical` (constantes `DELINQUENCY_FLOOR_HIGH_MIN_DAYS`/`DELINQUENCY_FLOOR_CRITICAL_MIN_DAYS`, commentées "configurable per-org en V2"). Le floor ne redescend **jamais** la bande brute (un compte déjà `high` sur ses propres signaux le reste même si sa délinquence est récente) et ne s'applique jamais à un compte non délinquent. `'critical'` ne peut donc jamais survenir sur un compte `is_delinquent=false` — intercepté par la branche `impayes` de `determineSegmentTypesV3` (`isDelinquent` évalué avant `churnRiskBand`) avant d'atteindre la comparaison de bande, OR ajouté par défensivité malgré tout.
- `calculate-scores/index.ts` (`scoreAccountPure`) : `delinquentDurationDays` calculé depuis `account.delinquent_since`/`now` (jours entiers, jamais négatif) ; floor appliqué après `calcChurnRiskV2`, jamais sur la branche `isChurned` (état figé D1, exempté par construction — ce chemin ne passe jamais par le floor). Floor effectivement appliqué → signal `delinquency_duration_floor` ajouté à `risk_signals_triggered` (**`points: 0`** — jamais compté deux fois dans `churn_risk_score`, le floor change la bande, pas le score additif). `MODEL_VERSION` → `'v3.1'` (modification de la formule de scoring, bump obligatoire).
- Consommateurs `churn_risk_band='high'` élargis à `'high'`/`'critical'` (partout où l'invariant "`'critical'` est strictement au-dessus de `'high'`, jamais un synonyme" impose l'ajout) : `churn-alert/index.ts` (`isEligibleForChurnAlert`, filtre SQL), `weekly-digest/index.ts` (`criticalCount`), `calculate-scores/index.ts` (`getPreviousPrimarySegment`, trigger dispatch webhook sortant). **Non touchés, vérifié safe** : `dashboard-api::computeAccountsAtRisk`, `get_portfolio_snapshot.at_risk_count`, `accounts_with_priority.priority_label`, `get-today-actions::computePriority` — tous déjà en OR direct avec `is_delinquent` (décision 3, audit délinquence 2026-08-06) ; `'critical'` n'existe que quand `is_delinquent=true`, donc déjà couverts sans modification.
- `accounts-api/index.ts` : `delinquent_since` exposé (liste et détail, déjà `select('*')` côté détail — ajout du champ SELECT côté liste et de la ligne dans la réponse JSON). Le pseudo-signal `delinquency_duration_floor` remonte déjà via `risk_signals_triggered`, sans changement backend supplémentaire.
- Frontend (`sentio-dev-frontend`) : `ChurnRiskBand` élargi (+`'critical'`), `CHURN_BAND_STYLE`/`churnPulseClass` — style dédié (fond plein, pas `/15`) pour rester visuellement distinct de `'high'`. `delinquentDurationDays`/`formatDelinquentDuration` (`scoring-display.ts`) — `null` → `'—'`, **jamais `'0 days'`** (S1 : durée inconnue ≠ délinquence démarrée aujourd'hui). `AccountDelinquentBadge` affiche la durée (`Past due · N days`) et un tooltip d'escalade (`fr.scores.delinquentFloorHigh`/`delinquentFloorCritical`) à partir de 15/45 jours connus. `delinquent_since` propagé de bout en bout (`AccountsApiItem`/`AccountsApiDetailItem` → `AccountListItem`/`AccountDetail`) jusqu'aux 3 points d'affichage (`Accounts.tsx`, `AccountDetail.tsx`, `AccountHeader.tsx`).
- CI (`supabase-deploy.yml`) : assertion `a9_delinquent_45d_not_critical` (soft — `delinquent_since` est nouveau sur ce projet dev, 0 ligne avec une date connue tant que `sync-stripe` n'a pas encore tourné avec ce code).

**Vérifié en direct sur le projet dev** : les deux ALTER TABLE (`delinquent_since` + CHECK constraints élargies) appliqués et confirmés via `information_schema.columns`/`pg_get_constraintdef` — `'critical'` accepté par les deux CHECK. État actuel du portefeuille avant ce lot : 875 comptes `is_delinquent=true` (17 `churned` — exemptés par construction, 694 `watch`, 164 jamais scorés) ; `delinquent_since` à 100% `NULL` pour tous (colonne neuve). Requête `a9` exécutée manuellement : 0 ligne, cohérent avec l'absence totale de date connue à ce stade.

**Non vérifié dans ce lot, explicitement reporté au prochain déploiement** : aucune mesure avant/après réelle de la distribution des bandes n'a pu être faite — `delinquent_since` ne se peuple qu'au prochain run `sync-stripe` (les changements de code ne sont pas encore déployés sur l'Edge Function live), et un run `calculate-scores` avec le nouveau floor doit suivre pour que `churn_risk_band='critical'` apparaisse la première fois. Le test négatif (compte délinquent 60 jours → doit résoudre à `critical`, jamais autre chose) est couvert unitairement (`scoring-v3.test.ts`, `calculate-scores-churn.test.ts`) mais pas rejoué contre une donnée live pour la même raison.

**Tests** : `scoring-v3.test.ts` — 8 nouveaux tests `applyDelinquencyBandFloor` (dont le test négatif 60 jours → `critical`). `mrr-engine.test.ts` — 10 nouveaux tests `computeDelinquentSinceCandidate`/`resolveDelinquentSince`. `calculate-scores-churn.test.ts` — 5 nouveaux tests mirror de la colle `scoreAccountPure` (durée + injection du signal floor). `accounts-api.test.ts` — 2 nouveaux tests passthrough `delinquent_since`. Frontend : `scoring-display.test.ts` (nouveau, 6 tests) — `delinquentDurationDays`/`formatDelinquentDuration`, dont le cas `null → '—'`. `npm run verify` vert des deux côtés (backend : 1036 tests ; frontend : 43 tests, `tsc --noEmit`, `npm run build`).
