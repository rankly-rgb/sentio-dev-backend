...
- **T003 — grille tarifaire confirmée (2026-07-27)** : `free=30, growth=200, scale=750, enterprise=NULL` (illimité, sur demande). Les valeurs PLACEHOLDER initiales (`10/100/500`) ont été remplacées dans la migration et dans les fixtures de tests (`pricing.test.ts`, `pricing-status.test.ts`, `sentio-billing-subscribe.test.ts`) une fois la décision produit actée.
