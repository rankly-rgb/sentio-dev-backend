...
**UI de paiement — décision actée le 2026-07-27** : le paiement Growth self-serve utilisera **Stripe Checkout Session** (page hébergée, redirection) — pas Stripe Elements. **Non implémenté** : dépend de la création préalable du compte Stripe Billing séparé (voir `docs/stripe-billing-setup.md`, checklist non complétée à ce jour). L'endpoint actuel reste **volontairement en l'état** (statut synchrone via l'API `Subscriptions` directe, `payment_behavior: default_incomplete`, aucun `client_secret`/aucune session Checkout retournée, pas de collecte de paiement réelle possible) jusqu'à ce que le compte Stripe Billing existe et que la Checkout Session soit implémentée en conséquence.
